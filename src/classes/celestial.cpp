...
void Map::generate_rocky_map(CelestialObject *cel)
{
    ...
    double edge_dist, mottle_strength, mottle_noise;
    unsigned int x, y;
    ...
    // Disabled for now (revisit later) -- flip to true to re-enable province coloring.
    const bool enable_provinces = false;
        ...
        for (x = 0; x < image_width; ++x)
        {
            ...
            if (enable_provinces)
            {
                ...
                grain_value = ridged_fBm(nx * scale * 14.0 + 91.7, ny * scale * 14.0 + 43.1, nz * scale * 14.0 + 17.9,
                    4, lacunarity, gain);
                r_weight = fmin(1.0, fmax(0.0, 0.7 + 0.6 * (grain_value - 0.5)));
