...
//-----------------------------------------------------------------------------
// About 'outer_size':
// Its meaning must differ slightly depending on if we are using ScrollX/ScrollY flags.
...
//   - inner_width  = 0.0f  ->  fit in outer_width: Fixed size columns will take space they require (if avail, otherwise shrink down), Stretch columns becomes Fixed columns.
//   - inner_width  > 0.0f  ->  override scrolling width, generally to be larger than outer_size.x. Fixed column take space they require (if avail, otherwise shrink down), Stretch columns share remaining space!
//-----------------------------------------------------------------------------
// Details:
// - If you want to use Stretch columns with ScrollX, you generally must specify 'inner_width' otherwise the concept
...
//
// - we distinguish those cases because non-hidden columns that are clipped outside of scrolling bounds should still contribute their height to the row.
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // - can't implement support for ImGuiChildFlags_ResizeY as we somehow pull the height data from somewhere. this also requires stable instance numbers.
    ...
    if (inner_window != outer_window)
    {
        ...
        // it already overlaps it and doesn't require an extra offset. Ideally we should be able to pass custom border size with
    ...
    table->InnerClipRect.ClipWith(table->WorkRect);     // We must set this to honor inner_width
    ...
    table->RowPosY1 = table->RowPosY2 = table->WorkRect.Min.y; // This is necessary somehow
    ...
    // Using opaque colors facilitate overlapping lines of the grid, otherwise we'd have to improve TableDrawBorders()
    ...
    // It will also react to changing fonts with mixed results. It doesn't have to be perfect but merely provide a decent transition.
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        // Release
        ImGuiContext& g = *GImGui;
        if (g.ActiveId == 0) // FIXME: really should revisit. See 38f5e5a.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // It would easily work without but we're not ready to guarantee it since e.g. names gotta be resubmitted anyway.
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        // Assign final width, record width in case we will want to shrink
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        {
            // As we store an absolute value to make per-cell updates faster, we offset values used for width computation.
    ...
    // This is done late to handle the case of fixed-columns tables not claiming more widths that they would use.
...
void    ImGui::EndTable()
{
    ...
    // Override declared contents width/height to enable auto-resize while not unnecessaryly adding a scrollbar
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    IM_ASSERT_USER_ERROR_RET(table->IsLayoutLocked == false, "TableSetupColumn(): make sure to call before first row!"); // Table layout is locked when submitting a row or when calling BeginMultiSelect() with box-select.
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT_USER_ERROR_RET(table != NULL, "Call should only be done while in BeginTable() scope!");
    IM_ASSERT(table->IsLayoutLocked == false && "TableSetupColumn(): make sure to call before first row!");
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        if (draw_cell_bg_color)
        {
            ...
            {
                // As we render the BG here we clip things (for layout we would not)
                // FIXME: This cancels the OuterPadding addition done by TableGetCellBgRect(), be sure to keep it while rendering correctly while scrolling.
    ...
    // - we do that in TableEndRow() instead of TableBeginRow() so the list clipper can mark
...
// This is called very frequently, so we be mindful of unnecessary overhead.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // When forwarding resize from Wn| to Fn+1| we be considerate of the _NoResize flag on Fn+1.
    ...
    // - W3 F1 F2  resize W3|               --> to not be stuck past F1|, both F1 and F2 would want to stretch down, which would be lossy or ambiguous. Seems hard to fix.
...
// - We allocate them following storage order instead of display order so reordering columns won't unnecessaryly
...
//   horizontal spanning. If we allowed vertical spanning we'd use one background draw channel per merge group (1-4).
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            // (note that we assume that rendering didn't stray on the left direction. we should use a CursorMinPos to detect it)
...
// FIXME-TABLE: This is a mess, let's redesign how we render borders (as some are also done in TableEndRow)
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    const bool must_fix_linearize = ((ImU64)1 << sort_order_count) != (sort_order_mask + 1);
    const bool must_fix_single_sort_order = (sort_order_count > 1) && !(table->Flags & ImGuiTableFlags_SortMulti);
    if (must_fix_linearize || must_fix_single_sort_order)
    {
        ImU64 fixed_mask = 0x00;
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            // Fix: Rewrite sort order fields if necessary so they have no gap or duplicate.
...
// The intent is that advanced users willing to create customized headers would not have to use this helper
...
void ImGui::TableHeadersRow()
{
    ...
    // it easier to debug-step in TableUpdateLayout(). Your own version of this function doesn't require this.
    if (!table->IsLayoutLocked)
        TableUpdateLayout(table);
...
void ImGui::TableHeader(const char* label)
{
    ...
    if (held && (table->Flags & ImGuiTableFlags_Reorderable) && IsMouseDragging(0) && !g.DragDropActive)
    {
        ...
        // - we handle reordering across hidden columns.
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        // This assume that reordering constraints output a single range, otherwise would either call TableGetMaxDisplayOrderAllowed() for each item below, or cache this once per frame into columns.
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        // we save the [Table] entry even if all the bools are false, since this records a table with "default settings".
...
{
    // Active (dragged) column always follow mouse. The reason we use this is that dragging a column to the right edge of an auto-resizing
...
{
    // We have few columns per window so for now we don't bother much with turning this into a faster lookup.
...
void ImGui::NextColumn()
{
    ...
    // (which would unnecessaryly attempt to update commands in the wrong channel, then pop or overwrite them),
