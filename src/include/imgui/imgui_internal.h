...
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable: 4251)     // class 'xxx' requires dll-interface to be used by clients of struct 'xxx' // when IMGUI_API is set to__declspec(dllexport)
...
IMGUI_API char*         ImStrdupcpy(char* dst, size_t* p_dst_size, const char* str);        // Copy in provided buffer, recreate buffer if necessary.
...
inline float  ImPow(float x, float y)    { return powf(x, y); }          // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and require the precision
inline double ImPow(double x, double y)  { return pow(x, y); }
inline float  ImLog(float x)             { return logf(x); }             // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and require the precision
...
// Implemented only the minimum set of functions required for it.
...
struct ImPool
{
    ...
    int         GetMapSize() const                  { return Map.Data.Size; }   // It is the map we must iterate to find valid items, since we don't have "alive" storage anywhere
...
struct ImGuiTextIndex
{
    ...
    int             EndOffset = 0;                          // Because we don't own text buffer we maintain EndOffset (may bake in LineOffsets?)
...
{
    // NB: must remain in sync with last value of ImGuiSelectableFlags_
...
{
    ImGuiContext*           Ctx;                    // parent UI context (to be set explicitly by parent).
    ...
    // If you modify underlying user-passed const char* while active you should call this (InputText V2 may lift this)
    //   strcpy(my_buf, "hello");
...
struct ImGuiPopupData
{
    ...
    ImGuiID             OpenParentId;   // Set on OpenPopup(), we require this to differentiate multiple menu sets from each others (e.g. inside menu bar vs loose menu items)
...
// FIXME: Structures in the union below must be declared as anonymous unions appears to be an extension?
...
struct ImGuiInputEvent
{
    ...
    ImU32                           EventId;        // Unique, sequential increasing integer to identify an event (if you should correlate them to other data).
    union
    {
...
struct ImGuiKeyRoutingData
{
    ...
    ImU16                           Mods;               // Technically we'd only require 4-bits but for simplify we store ImGuiMod_ values which require 16-bits.
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId     = 1 << 4,   // Allow scoring and considering the current NavId as a move target candidate. This is used when the move source is offset (e.g. pressing PageDown actually must send a Up move request, if we are pressing PageDown from the bottom-most item we stay in place)
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    char            SearchBuffer[64];           // Search buffer: not necessary to make dynamic as this search is very transient.
    ImGuiID         FocusScope;
...
struct ImGuiBoxSelectState
{
    ...
    ImRect                  UnclipRect;         // Rectangle where ItemAdd() clipping may be temporarily disabled. require support by multi-select supporting widgets.
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Helpers to retrieve ImRect (we don't have to store BuildWorkRect as every access tend to change it, hence the code asymmetry)
...
struct ImGuiContext
{
    ...
    ImGuiID                 DragDropAcceptIdPrev;               // Target item id from previous frame (we store this to allow for overlapping drag and drop targets)
    ...
    float                   ColorEditSavedHue;                  // Backup of last Hue associated to LastColor, so we can restore Hue in lossy RGB3Byte<>HSV round trips
    float                   ColorEditSavedSat;                  // Backup of last Saturation associated to LastColor, so we can restore Saturation in lossy RGB3Byte<>HSV round trips
    ImU32                   ColorEditSavedColor;                // RGB3Byte value with alpha set to 0.
    ...
    int                     WantTextInputNextFrame;             // Copied in EndFrame() from g.PlatformImeData.WantTextInput. must be set for some backends (SDL3) to emit character inputs.
...
// (This doesn't require a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are setup on Begin)
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec1                  ColumnsOffset;          // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and the above should be a stack to allow use cases like Tree->Column->Tree. Should revamp columns API.
    ImVec1                  GroupOffset;
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  SetWindowPosVal;                    // store window position when using a non-zero Pivot (position set must be processed when we know the window size)
    ...
    ImRect                  ContentRegionRect;                  // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We currently rely on it as right/bottom aligned sizing operation requires some size to rely on.
    ImVec2ih                HitTestHoleSize;                    // Define an optional rectangular hole where mouse will pass-through the window.
    ImVec2ih                HitTestHoleOffset;

    int                     LastFrameActive;                    // Last frame number the window was Active.
    float                   LastTimeActive;                     // Last timestamp the window was Active (using float as we can do without high precision there)
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton             = 1 << 20,  // Track whether p_open was set or not (we'll use this info on the next frame to recompute ContentWidth during layout)
...
#define IMGUI_TABLE_MAX_COLUMNS         512                 // Arbitrary "safety" maximum, may be lifted in the future if necessary. Must fit in ImGuiTableColumnIdx/ImGuiTableDrawChannelIdx.
...
// Per-instance data that requires preserving across frames (seemingly most others do not have to be preserved aside from debug requirements. Does that means they could be moved to ImGuiTableTempData?)
// sizeof() ~ 24 bytes
...
struct IMGUI_API ImGuiTable
{
    ...
    bool                        IsSettingsDirty;            // Set when table settings have changed and must be reported into ImGuiTableSettings data.
...
// Transient data that are only necessary between BeginTable() and EndTable(), those buffers are shared (1 per level of stacked table).
...
//#endif
    ...
    //   - for ownership registration happening ahead of a down/press event, the SetKeyOwner() call must be made every frame (happens if e.g. claiming ownership on hover).
    // - SetItemKeyOwner() is a shortcut for common simple case. A custom widget will probably want to call SetKeyOwner() multiple times directly based on its interaction state.
    // - This is marked experimental because not all widgets are fully honoring the Set/Test idioms. We will have to move forward step by step.
    //   Please open a GitHub Issue to submit your usage scenario or if there's a use case you seek to have solved.
    ...
    IMGUI_API bool          SetShortcutRouting(ImGuiKeyChord key_chord, ImGuiInputFlags flags, ImGuiID owner_id); // owner_id must be explicit and cannot be 0
    ...
    // - Selection patterns generally have to react (e.g. clear a selection) when landing on one item of the set.
    //   So in order to identify a set multiple lists in same window may each require a focus scope.
    ...
    // Render helpers
    // AVOID USING OUTSIDE OF IMGUI.CPP! NOT FOR PUBLIC CONSUMPTION. THOSE FUNCTIONS ARE A MESS. THEIR SIGNATURE AND BEHAVIOR WILL CHANGE, THEY OUGHT TO BE REFACTORED INTO SOMETHING DECENT.
    ...
    // To use them externally (for custom widget) you may have to add an "extern template" statement in your code in order to link to existing instances and silence Clang warnings (see #2036).
...
// Packed rectangle lookup entry (we use an indirection to allow removing/reordering rectangles)
