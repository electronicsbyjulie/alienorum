...
// Those MIN/MAX values are not define because we point to them
...
{
    ImGuiContext& g = *GImGui;
    const bool require_backup = (g.CurrentWindow->DC.TextWrapPos < 0.0f);  // Keep existing wrap position if one is already set
    if (require_backup)
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        // - Technically we only use some values in one code path, but since this is gated by hovered test this is fine.
        ...
        if (mods_ok)
        {
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                ...
                if (flags & (ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnClickReleaseAnywhere))
                {
                    ...
                    {
                        FocusWindow(window, ImGuiFocusRequestFlags_RestoreFocusedChild); // Still important to focus and bring to front, but try to avoid losing NavId when navigating a child
                    }
                }
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    IM_ASSERT(ImMax(size_contents_v, size_visible_v) > 0.0f); // Adding this assert to check if the ImMax(XXX,1.0f) is still necessary. PLEASE CONTACT ME if this triggers.
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // We currently don't provide the same feature for tables because tables naturally have border features.
        ...
        // In order to handle scaling we scale separator thickness and it would not makes sense to have a disparity depending on height.
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    g.NextWindowData.ClearFlags(); // We behave like Begin() and consume those values
    if (window->SkipItems)
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ...
    // The old Combo() API exposed "popup_max_height_in_items". The new more general BeginCombo() API doesn't have/require it, but we emulate it here.
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format, void* p_data_when_empty)
{
    ...
    // Small types take a 32-bit buffer to receive the result from scanf()
    int v32 = 0;
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (is_logarithmic)
    {
        // When using logarithmic sliders, we clamp to avoid hitting zero, but our choice of clamp value greatly affects slider precision. We attempt to use the specified precision to estimate a good lower bound.
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (logarithmic_zero_epsilon > 0.0f) // == is_logarithmic from caller
    {
        ...
        // Awkward special cases - we use ranges of the form (-100 .. 0) to convert to (-100 .. -epsilon), not (-100 .. epsilon)
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (logarithmic_zero_epsilon > 0.0f) // == is_logarithmic from caller
    {
        ...
        // Awkward special case - we use ranges of the form (-100 .. 0) to convert to (-100 .. -epsilon), not (-100 .. epsilon)
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const bool is_logarithmic = (flags & ImGuiSliderFlags_Logarithmic) != 0;
    const bool is_floating_point = (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
    const float v_range_f = (float)(v_min < v_max ? v_max - v_min : v_min - v_max); // It doesn't take high precision for what we do with it.
...
const char* ImParseFormatTrimDecorations(const char* fmt, char* buf, size_t buf_size)
{
    ...
    if (fmt_end[0] == 0) // If we only have leading decoration, we don't have to copy the data.
        return fmt_start;
...
// - For scanning we remove all width and precision fields and flags "%+3.7f" -> "%f". BUT don't strip types like "%I64d" which includes digits. ! "%07I64d" -> "%I64d"
...
// FIXME: This is still used by some navigation code path to infer a minimum tweak step, but we should aim to rework widgets so it isn't necessary.
...
{
    // FIXME: May have to clarify display behavior if format doesn't contain %.
...
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline() or  InputTextEx() manually if you want multi-line + hint.
...
#define STB_TEXTEDIT_MOVEWORDLEFT       STB_TEXTEDIT_MOVEWORDLEFT_IMPL  // They should be #define for stb_textedit.h
...
// we should implement some form of hook to reapply data back to user buffer on deactivation frame. (#4714)
...
// 'max_output_buffer_size' happens to be a meaningful optimization to avoid writing the full line_index when not necessarily necessary (e.g. very large buffer, scrolled up, inactive)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        // Support for internal ImGuiInputTextFlags_MergedItem flag, which could be redesigned as an ItemFlags if necessary (with test performed in ItemAdd)
        ItemSize(total_bb, style.FramePadding.y);
    ...
    if (g.ActiveId == id)
    {
        ...
        // FIXME: The reason we don't use Shortcut() is we would use a routing flag to specify multiple mods, or to all mods combination into individual shortcuts.
    ...
    if (g.ActiveId == id)
    {
        ...
        // We ignore Ctrl inputs, but ought to allow Alt+Ctrl as some keyboards (e.g. German) use AltGR (which _is_ Alt+Ctrl) to input certain characters.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_enter || is_ctrl_enter || is_shift_enter || is_gamepad_validate)
        {
            ...
            if (!is_new_line)
            {
                ...
                if (io.ConfigInputTextEnterKeepActive && !is_multiline)
                {
                    ...
                    state->SelectAll(); // Not necessary to scroll
        ...
        else if (is_cut || is_copy)
        {
            ...
            {
                // SetClipboardText() only takes null terminated strings + state->TextSrc may point to read-only user buffer, so we make a copy.
    ...
    if (g.ActiveId == id)
    {
        ...
        if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
        {
            ...
            // The reason we specify the usage semantic (Completion/History) is that Completion must disable keyboard TABBING at the moment.
            ...
            if (event_flag)
            {
                ...
                // FIXME-OPT: Undo stack reconcile could use a backup of the data until we rework API, see #7925
                ...
                if (callback_data.BufDirty)
                {
                    ...
                    IM_ASSERT(callback_data.BufTextLen == (int)ImStrlen(callback_data.Buf)); // you should maintain BufTextLen if you change the text!
    ...
    // At this point we handle the possibility that a callback could have modified the underlying buffer (#8368)
    ...
    // Store text height (we don't use width)
    ...
    if (render_cursor || render_selection)
    {
        ...
        // This is going to be messy. we:
        // - Display the text (this alone can be more easily clipped)
        // - Handle scrolling, highlight selection, display cursor (those all requires some form of 1d->2d cursor position calculation)
        // - Measure text height (for scrollbar)
        // We are attempting to do most of that in **one main pass** to minimize the computation cost (non-negligible for large amount of text) + 2nd pass for selection rendering (we could merge them by an extra refactoring effort)
        // FIXME: This should occur on buf_display but we'd make sure to maintain cursor/select_start/select_end for UTF-8.
        IM_ASSERT(state != NULL);
    ...
    if (is_multiline)
    {
        // For focus requests to work on our multiline we ensure our child ItemAdd() call specifies the ImGuiItemFlags_Inputable (see #4761, #7870)...
        ...
        // ...and then we undo the group overriding last item data, which gets a bit messy as EndGroup() tries to forward scrollbar being active...
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Convert to the formats we will be using
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        // but we use SameLine() to setup baseline correctly. Might want to refactor SameLine() to simplify this.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding, 0, 1.0f * (float)(int)g.Style._MainScale); // Color buttons often benefit from having some sort of border // FIXME-DPI
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (!is_visible)
    {
        if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) && (window->DC.TreeRecordsClippedNodesY2Mask & (1 << (window->DC.TreeDepth - 1))))
        {
            ...
            parent_data->DrawLinesToNodesY2 = ImMax(parent_data->DrawLinesToNodesY2, window->DC.CursorPos.y); // Don't have to aim to mid Y position as we are clipped anyway.
    ...
    const bool is_multi_select = (g.LastItemData.ItemFlags & ImGuiItemFlags_IsMultiSelect) != 0;
    if (is_multi_select) // We absolutely must distinguish open vs select so _OpenOnArrow comes by default
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        // FIXME: CloseButton can overlap into text, should find a way to clip the text somehow.
        ImGuiContext& g = *GImGui;
...
// But you should make sure the ID is unique, e.g. enclose calls in PushID/PopID or use ##unique_id.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        //   - (1) it would require focus scope to be set, require exposing PushFocusScope() or equivalent (e.g. BeginSelection() calling PushFocusScope())
    ...
    // selection toggle, only useful if you want that state updated (e.g. for rendering purpose) before reaching EndMultiSelect().
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    for (ImWchar w : g.IO.InputQueueCharacters)
    {
        ...
        {
            select_request = true; // Same character: don't have to append to buffer.
            continue;
        }
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window, ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags)
{
    ...
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
    {
        ...
        if (bs->UnclipMode && g.CurrentTable != NULL)
            TableApplyExternalUnclipRect(g.CurrentTable, bs->UnclipRect); // Not necessary submitting both
    }
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (ms->IsFocused)
    {
        ...
        // - Box select also handle Escape and must pass an id to bypass ActiveIdUsingAllKeyboardKeys lock.
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags)
{
    ...
    if (ms->IsFocused)
    {
        ...
        // If you are using a clipper you should process the SetAll request after calling BeginMultiSelect()
        ...
        // For this to work, someone should set 'RangeSrcPassedBy = true' at some point (either clipper either SetNextItemSelectionUserData() function)
        if (ms->IsKeyboardSetRange)
        {
    ...
    // To handle drag and drop of multiple items we avoid clearing selection on click.
...
//   - Your code may do differently. If you store pointers or objects ID in ImGuiSelectionUserData you may have to perform
...
// If you can do without a label you can pretty much directly use ImGui::BeginChild() with ImGuiChildFlags_FrameStyle.
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    // FIXME-OPT: We could omit the BeginGroup() if label_size.x == 0.0f but would have to omit the EndGroup() as well.
    BeginGroup();
    if (label_size.x > 0.0f)
    {
...
bool ImGui::ListBox(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int height_in_items)
{
    ...
    // Assume all items have even height (= 1 line of text). If you will be creating items of different height,
    // you can create a custom version of ListBox() in your code without using the clipper.
    bool value_changed = false;
    ImGuiListClipper clipper;
    clipper.Begin(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line height here so we pass it as a minor optimization, but generally you can live without doing so.
    clipper.IncludeItemByIndex(*current_item);
    while (clipper.Step())
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    // This is currently not exposed in the public API because we hope to implement a larger redesign of the whole thing, but in the short-term we are making it available in PlotEx().
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return idx_hovered;
}
...
bool ImGui::BeginMenuBar()
{
    ...
    // We overwrite CursorMaxPos because BeginGroup sets it to CursorPos (essentially the .EmitItem hack in EndMenuBar() would do well with something analogous here, maybe a BeginGroupEx() with flags).
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        // But we also must not close the top-menu menu when moving over void. Perhaps we should extend the triangle check to a larger polygon.
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    // Detect if we sort out tab list (e.g. in rare case where a tab changed section)
    int tab_dst_n = 0;
    bool require_sort_by_section = false;
    ...
    for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++)
    {
        ...
        // We will apply sorting if tabs have changed section (e.g. moved from one of Leading/Central/Trailing to another)
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // Store data so we can build an array sorted by width if we shrink tabs down
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    ImGuiTabBar* tab_bar = g.CurrentTabBar;
    IM_ASSERT_USER_ERROR_RETV(tab_bar != NULL, false, "must be called between BeginTabBar() and EndTabBar()!");
...
void    ImGui::EndTabItem()
{
    ...
    ImGuiTabBar* tab_bar = g.CurrentTabBar;
    IM_ASSERT_USER_ERROR_RET(tab_bar != NULL, "must be called between BeginTabBar() and EndTabBar()!");
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so it's important to add an extra draw call (temporary in the case of vertical animation)
...
// To use it to make sure to call the function SetTabItemClosed() between BeginTabBar() and EndTabBar().
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // But right now if you want to alter text color of tabs this is what you should do.
#if 0
