...
//     constructors if you like. You will have to implement ==/!= operators.
// History:
...
// Define this if you should change the invalid value for your backend.
// - If your backend is using ImTextureID to store an index/offset and you require 0 to be valid, You can add '#define ImTextureID_Invalid ((ImTextureID)-1)' in your imconfig.h file.
...
namespace ImGui
{
    ...
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will make sure to call SetCurrentContext() + SetAllocatorFunctions()
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't have to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't have to render, better to not create any windows and not call NewFrame() at all!
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window position in screen space (IT IS UNLIKELY YOU EVER have to USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    IMGUI_API ImVec2        GetWindowSize();                            // get current window size (IT IS UNLIKELY YOU EVER have to USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    IMGUI_API float         GetWindowWidth();                           // get current window width (IT IS UNLIKELY YOU EVER have to USE THIS). Shortcut for GetWindowSize().x.
    IMGUI_API float         GetWindowHeight();                          // get current window height (IT IS UNLIKELY YOU EVER have to USE THIS). Shortcut for GetWindowSize().y.
    ...
    // - YOU CAN DO 99% OF WHAT YOU require WITH ONLY GetCursorScreenPos() and GetContentRegionAvail().
    // - Attention! We currently have inconsistencies between window-local and absolute positions we will aim to fix with future API:
    //    - Absolute coordinate:        GetCursorScreenPos(), SetCursorScreenPos(), all ImDrawList:: functions. -> this is the preferred way forward.
    //    - Window-local coordinates:   SameLine(offset), GetCursorPos(), SetCursorPos(), GetCursorStartPos(), PushTextWrapPos()
    //    - Window-local coordinates:   GetContentRegionMax(), GetWindowContentRegionMin(), GetWindowContentRegionMax() --> all obsoleted. YOU DON'T require THEM.
    ...
    IMGUI_API void          TextWrapped(const char* fmt, ...)                               IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may have to set a size using SetNextWindowSize().
    ...
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse must move by 5 pixels to increase value by 1). For keyboard/gamepad navigation, minimum speed is Max(v_speed, minimum_step_at_given_precision).
    ...
    // Widgets: Trees
    // - TreeNode functions return true when the node is open, in which case you should also call TreePop() when you are finished displaying the tree node contents.
    ...
    // - TreeNode() is technically supported but... using this correctly is more complicated. You require some sort of linear/random access to your tree,
    ...
    IMGUI_API bool                  IsItemToggledSelection();                                   // Was the last item selection state toggled? Useful if you require the per-item information _before_ reaching EndMultiSelect(). We only returns toggle _event_ in order to handle clipping correctly.

    // Widgets: List Boxes
    // - This is essentially a thin wrapper to using BeginChild/EndChild with the ImGuiChildFlags_FrameStyle flag for stylistic changes + displaying a label.
    // - If you don't require a label you can probably simply use BeginChild() with the ImGuiChildFlags_FrameStyle flag for the same result.
    ...
    //  - The 3 properties above are related: we retain popup visibility state in the library because popups may be closed as any time.
    //  - You can bypass the hovering restriction by using ImGuiHoveredFlags_AllowWhenBlockedByPopup when calling IsItemHovered() or IsWindowHovered().
    //  - IMPORTANT: Popup identifiers are relative to the current ID stack, so OpenPopup and BeginPopup generally must be at the same level of the stack.
    ...
    //  - CloseCurrentPopup() is called by default by Selectable()/MenuItem() when activated (FIXME: require some options).
    ...
    //      TableNextColumn() will automatically wrap-around into the next row if necessary.
    //    - IMPORTANT: Comparatively to the old Columns() API, we call TableNextColumn() for the first column!
    ...
    // Overlapping mode
    IMGUI_API void          SetNextItemAllowOverlap();                                          // allow next item to be overlapped by a subsequent item. Typically useful with InvisibleButton(), Selectable(), TreeNode() covering an area where subsequent items may have to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_NoUndoRedo          = 1 << 16,  // Disable undo/redo. Note that input text owns the text data while active, if you want to provide your own undo/redo stack you require e.g. to call ClearActiveID().
    ...
    ImGuiInputTextFlags_CallbackResize      = 1 << 22,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their Size). You will be provided a new BufSize in the callback and have to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
    ...
    // - Wrapping width must always account for the possibility of a vertical scrollbar.
...
enum ImGuiHoveredFlags_
{
    ...
    // - use those if you require specific overrides.
    ImGuiHoveredFlags_Stationary                    = 1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After this, can move on same item/window. Using the stationary test tends to reduces the require for a long delay.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to test if the payload must be delivered.
...
enum ImGuiKey : int
{
    ...
    // [Internal] If you should iterate all keys (for e.g. an input mapper) you may use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_RouteOverActive         = 1 << 15,  // Option: global route: higher priority than active item. Unlikely you should use that: will interfere with every active items, e.g. Ctrl+A registered by InputText will be overridden by this. May not be fully honored as user/internal code is likely to always assume they can access keys when active.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NavEnableGamepad       = 1 << 1,   // Master gamepad navigation enable flag. Backend also must set ImGuiBackendFlags_HasGamepad.
...
//   - In Visual Studio w/ Visual Assist installed: Alt+G ("VAssistX.GoToImplementation") can also follow symbols inside comments.
//   - In VS Code, CLion, etc.: Ctrl+Click can follow symbols inside comments.
// - When changing this enum, you should update the associated internal table GStyleVarInfo[] accordingly. This is where we link enum values to members offset/type.
enum ImGuiStyleVar_
{
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_DisplayRGB      = 1 << 20,  // [Display]    // ColorEdit: override _display_ type among RGB3Byte/HSV/Hex. ColorPicker: select any combination using one or more of RGB3Byte/HSV/Hex.
    ...
    ImGuiColorEditFlags_InputRGB        = 1 << 27,  // [Input]      // ColorEdit, ColorPicker: input and output data in RGB3Byte format.
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_InvalidMask_       = 0x7000000F,   // [Internal] We treat using those bits as being potentially a 'float power' argument from legacy API (obsoleted 2020-08) that has got miscast to this enum, and will trigger an assert if necessary.
};
...
//    - Fixed Columns can be enlarged as necessary. Table will show a horizontal scrollbar if necessary.
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Reorderable                = 1 << 1,   // Enable reordering columns in header row. (require calling TableSetupColumn() + TableHeadersRow() to display headers, or using ImGuiTableFlags_ContextMenuInBody to access context-menu without headers).
...
//-----------------------------------------------------------------------------
// - You generally do NOT have to care or use this ever. But we make it available in imgui.h because some of our public structures are relying on it.
...
struct ImGuiStyle
{
    ...
    // Behaviors
    // (It is possible to modify those fields mid-frame if specific behavior require it, unlike e.g. configuration fields in ImGuiIO)
...
struct ImGuiIO
{
    ...
    // - By design, we do NOT allow error recovery to be 100% silent. One of the three options must be checked!
    ...
    // - Presently Begin()/End() and BeginChild()/EndChild() must ALWAYS be called in tandem, regardless of return value of BeginXXX()
    // - This is inconsistent with other BeginXXX functions and create confusion for many users.
    // - We expect to update the API eventually. In the meanwhile we provide tools to facilitate checking user-code behavior.
    bool        ConfigDebugBeginReturnValueOnce;// = false          // First-time calls to Begin()/BeginChild() will return false. must BE SET AT APPLICATION BOOT TIME if you don't want to miss windows.
    ...
    ImGuiContext* Ctx;                              // Parent UI context (must be set explicitly by parent).
...
struct ImGuiInputTextCallbackData
{
    ...
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. you should update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal state.
...
// NB: For basic min/max size constraint on each axis you don't have to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
struct ImGuiSizeCallbackData
{
    void*   UserData;       // Read-only.   What user passed to SetNextWindowSizeConstraints(). Generally store an integer or float in here (require reinterpret_cast<>).
...
struct ImGuiStorage
{
    ...
    // - Sorted insertion is costly, paid once. A typical frame shouldn't have to insert any new pair.
...
// (Dear ImGui already clip items based on their bounds but: it must first layout the item to do so, and generally
...
struct ImGuiListClipper
{
    ...
    // items_count: Use INT_MAX if you don't know how many items you have (in which case the cursor won't be advanced in the final step, and you can call SeekCursorForItem() manually if you require)
    ...
    // Call IncludeItemByIndex() or IncludeItemsByIndex() *BEFORE* first call to Step() if you require a range of items to not be clipped, regardless of their visibility.
...
// - Any setting other than the default will require custom backend support. The only standard backend that supports anything else than the default is DirectX9.
...
// **None of the ImGui API are using ImColor directly but you can use it as a convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly cast to ImU32 or ImVec4 if necessary.
struct ImColor
{
    ImVec4          Value;
    ...
    // FIXME-OBSOLETE: May have to obsolete/cleanup those helpers.
...
//           - (3) [If using clipper] you should make sure RangeSrcItem is always submitted. Calculate its index and pass to clipper.IncludeItemByIndex(). If storing indices in ImGuiSelectionUserData, a simple clipper.IncludeItemByIndex(ms_io->RangeSrcItem) call will work.
...
//   SetRange requests will give you two end-points and you will have to iterate/interpolate between them to update your selection.
...
// - To use 16-bit indices + allow large meshes: backend have to set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset (recommended).
...
// NB: You most likely do NOT have to use draw callbacks just to create your own widget or customized UI rendering,
...
// The code expect ImVec2 pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order them or add other fields as necessary to simplify integration in your engine.
...
// NOTE: IMGUI DOESN'T CLEAR THE STRUCTURE AND DOESN'T CALL A CONSTRUCTOR SO ANY CUSTOM FIELD WILL BE UNINITIALIZED. IF YOU ADD EXTRA FIELDS (SUCH AS A 'Z' COORDINATES) YOU WILL have to CLEAR THEM DURING RENDER OR TO IGNORE THEM.
...
struct ImDrawList
{
    ...
    //   Use AddNgon() and AddNgonFilled() functions if you should guarantee a specific number of sides.
    ...
    //   - If userdata_size > 0,  we copy/store 'userdata_size' bytes pointed to by 'userdata'. We store them in a buffer stored inside the drawlist. ImDrawCmd::UserCallbackData will point inside that buffer so you have to retrieve data from there. Your callback may have to use ImDrawCmd::UserCallbackDataSize if you expect dynamically-sized data.
    ...
    IMGUI_API void  AddDrawCmd();                                               // This is useful if you should forcefully create a new draw call (to allow for dependent rendering / blending). Otherwise primitives are merged into the same draw-call as much as possible
    ...
    // - All primitives must be reserved via PrimReserve() beforehand.
...
struct ImDrawData
{
    ...
    ImVector<ImTextureData*>* Textures;     // List of textures to update. Most of the times the list is shared by all ImDrawData, has only 1 texture and it doesn't require any update. This almost always points to ImGui::GetPlatformIO().Textures[]. May be overridden or set to NULL if you want to manually update textures.
...
// FOR ALL OTHER ImTextureXXXX TYPES: ONLY CORE LIBRARY AND RENDERER BACKENDS have to KNOW AND CARE ABOUT THEM.
//-----------------------------------------------------------------------------
...
// This is only useful for (1) core library and (2) backends. End-user/applications do not have to care about this.
...
// - void*          BackendUserData = higher-level opaque storage for backend own book-keeping. Some backends may have enough with TexID and not require both.
...
struct ImFontConfig
{
    ...
    bool            FontDataOwnedByAtlas;   // true     // TTF/OTF data ownership taken by the owner ImFontAtlas (will delete memory itself). SINCE 1.92, THE DATA must PERSIST FOR WHOLE DURATION OF ATLAS.
    ...
    const ImWchar*  GlyphRanges;            // NULL     // *LEGACY* THE ARRAY DATA must PERSIST AS LONG AS THE FONT IS ALIVE. Pointer to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list).
...
//  - One or more fonts.
//  - Custom graphics data necessary to render the shapes necessary by Dear ImGui.
...
// Common pitfalls:
// - If you pass a 'glyph_ranges' array to AddFont*** functions, you should make sure that your array persists up until the
...
struct ImFontAtlas
{
    ...
    //   the texture (e.g. when using the AddCustomRect*** api), then the RGB3Byte pixels emitted will always be white (~75% of memory/bandwidth waste).
    // - From 1.92 with backends supporting ImGuiBackendFlags_RendererHasTextures:
    //   - Calling Build(), GetTexDataAsAlpha8(), GetTexDataAsRGBA32() is not necessary.
    ...
    int                         TexGlyphPadding;    // FIXME: Should be called "TexPackPadding". Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0 (will also have to set AntiAliasedLinesUseTex = false).
    ...
    ImFontAtlasBuilder*         Builder;            // Opaque interface to our data that doesn't have to be public and may be discarded when rebuilding.
...
struct ImFont
{
    ...
    bool                        EllipsisAutoBake;   // 1     //     // Mark when the "..." glyph (== EllipsisChar) must be generated by combining multiple '.'.
...
namespace ImGui
{
    ...
    // You do not require those functions! See #7838 on GitHub for more info.
    IMGUI_API ImVec2    GetContentRegionMax();                                  // Content boundaries max (e.g. window boundaries including scrolling, or current column boundaries). You should never require this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
    IMGUI_API ImVec2    GetWindowContentRegionMin();                            // Content boundaries min for the window (roughly (0,0)-Scroll), in window-local coordinates. You should never require this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
    IMGUI_API ImVec2    GetWindowContentRegionMax();                            // Content boundaries max for the window (roughly (0,0)+Size-Scroll), in window-local coordinates. You should never require this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
...
// - ImFontAtlasCustomRect::GlyphColored --> if you should write to this, instead you can write to 'font->Glyphs.back()->Colored' after calling AddCustomRectFontGlyph()
