...
// This can only be used along with a Renderer (e.g. DirectX11, OpenGL3, Vulkan..)
...
//  [X] Platform: Basic IME support. App calls 'SDL_SetHint(SDL_HINT_IME_SHOW_UI, "1");' before SDL_CreateWindow()!.

// You can use unmodified imgui_impl_* files in your project. See examples/ folder for examples of using this.
// Prefer including the entire imgui/ repository into your project (either as a copy or as a submodule), and only build the backends you will use.
...
//  2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user must call CreateContext/DestroyContext themselves.
...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer, void* sdl_gl_context)
{
    ...
    // It is unlikely to make a difference, but if your app absolutely must ignore the initial on-focus click:
    ...
    // For the main window to be affected, your application must call this manually before calling SDL_CreateWindow().
...
// - On Windows the process will fail unless marked DPI-aware!! SDL2 doesn't do it by default. You can call ::SetProcessDPIAware() or call ImGui_ImplWin32_EnableDpiAwareness() from Win32 backend.
