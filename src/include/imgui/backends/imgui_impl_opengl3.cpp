...
// This can only be used along with a Platform Backend (e.g. GLFW, SDL, Win32, custom..)
...
// About WebGL/ES:
// - Make sure to '#define IMGUI_IMPL_OPENGL_ES2' or '#define IMGUI_IMPL_OPENGL_ES3' to use WebGL or OpenGL ES.
// - This is done automatically on iOS, Android and Emscripten targets.
// - For other targets, the define must be visible from the imgui_impl_opengl3.cpp compilation unit. If unsure, define globally or in imconfig.h.
...
// - You may have to regenerate imgui_impl_opengl3_loader.h to add new symbols. See https://github.com/dearimgui/gl3w_stripped
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        GL_CALL(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
#else
        // GL ES doesn't have GL_UNPACK_ROW_LENGTH, so we either (A) copy to a contiguous buffer or (B) upload line by line.
        ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
        for (ImTextureRect& r : tex->Updates)
        {
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    */
    ...
    // Make an arbitrary GL call (we won't actually use the result)
