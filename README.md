...
* Package it with `assets/`, the small tracked `catalogs/*` files, and a starter set of texture maps (see
  below) into an NSIS installer, written to `package/Alienorum-<version>-win64.exe`.
...
(currently the Moon, Mars, Earth, Pluto, Charon, and Uranus's rings). The build script regenerates
`package/release-maps/` from `git ls-files maps/` on every run, so adding a new texture to that bundle is
just a matter of `git add`-ing it under `maps/` -- no changes to the packaging scripts required.